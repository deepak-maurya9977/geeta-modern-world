...
export default function HomeScreen({ onSelectTeaching }: HomeScreenProps) {
  ...
  return (
    <div className="min-h-screen saffron-bg relative">
      {/* Grain overlay */}
      <div className="grain-overlay" />

      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 z-50 px-4 md:px-8 py-4 flex justify-between items-center nav-saffron">
        <div className="flex items-center gap-2">
          <div className="saffron-mandala" style={{ width: 28, height: 28 }} />
          <span className="text-sm font-medium tracking-wide text-[#2C1810]">
            {lang === 'hi' ? 'ज्ञान संग्रह' : 'Wisdom Library'}
          </span>
        </div>
        <button
      ...
      <section className="pt-28 pb-10 px-4 md:px-8 text-center">
        <div className="saffron-mandala mx-auto mb-5" />
        <p className="eyebrow mb-2 text-[#D6A23A]">
          {lang === 'hi' ? 'आधुनिक पुनर्कथन' : 'Modern Retellings'}
        </p>
        <h1 className="text-4xl md:text-5xl lg:text-6xl text-[#2C1810] mb-4 max-w-2xl mx-auto leading-tight">
          {lang === 'hi' ? 'प्राचीन ज्ञान,' : 'Ancient Wisdom,'}
          <br />
          {lang === 'hi' ? 'आधुनिक जीवन' : 'Modern Lives'}
        </h1>
        <p className="text-base md:text-lg text-[#8B7355] max-w-xl mx-auto">
          {lang === 'hi'
      ...
      <div className="saffron-divider mb-10" />
      ...
      <section className="px-4 md:px-8 pb-20 max-w-6xl mx-auto">
        ...
        {/* Footer note */}
        <p className="text-center text-xs text-[#8B7355]/50 mt-12">
          {lang === 'hi'
  ...
  return (
    <div
      ...
      className={`group relative overflow-hidden rounded-3xl border transition-all duration-500 ${
        isComingSoon
          ? 'border-[#D6A23A]/10 cursor-default opacity-85'
          : 'border-[#D6A23A]/20 cursor-pointer hover:border-[#D6A23A]/50 hover:shadow-2xl hover:-translate-y-1'
      }`}
      style={{ background: 'rgba(253,246,236,0.7)', backdropFilter: 'blur(8px)' }}
    ...
    >
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          {isComingSoon ? (
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider bg-[#2C1810]/70 text-[#F4EFE6]/70 backdrop-blur-sm border border-[#F4EFE6]/10">
            ...
            <span
              ...
              style={{
                ...
                border: `1px solid ${teaching.accentColor}44`,
              }}
            >
              {tag}
            </span>
        ...
        {/* Arrow on hover (available only) */}
        {!isComingSoon && (
          <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
            <div
              className="w-9 h-9 rounded-full flex items-center justify-center"
              style={{ backgroundColor: teaching.accentColor }}
            >
      ...
      <div className="p-5">
        ...
        <div
          ...
          style={{ backgroundColor: isComingSoon ? '#8B7355' : teaching.accentColor }}
        />

        <h2 className="text-xl font-serif text-[#2C1810] mb-2 leading-snug">
          {title}
        </h2>
        <p className="text-sm text-[#8B7355] leading-relaxed line-clamp-3">
          {desc}
        </p>
        ...
        {!isComingSoon && (
          <div
            ...
            style={{ color: teaching.accentColor }}
          >
        ...
        {isComingSoon && (
          <p className="mt-4 text-xs text-[#8B7355]/50 italic">
